...
locals {
  ...
  instance_type              = var.k8s_config.control_plane_instance_type
  controller_on_demand_count = var.k8s_config.controller_on_demand_count
  controller_spot_count      = var.k8s_config.controller_spot_count
  ...
  security_group_ids = split(",", var.aws_config.security_group_ids)
  subnet_ids = split(",", var.aws_config.subnet_ids)
...
resource "aws_autoscaling_group" "controller_asg" {
  count = (local.controller_on_demand_count + local.controller_spot_count) > 0 ? 1 : 0

  name_prefix = "${local.cluster_name}-controller-asg-"
  desired_capacity = local.controller_on_demand_count + local.controller_spot_count
  min_size         = local.controller_on_demand_count + local.controller_spot_count
  max_size         = local.controller_on_demand_count + local.controller_spot_count
...
locals {
  asg_instance_ips = (local.controller_on_demand_count + local.controller_spot_count) > 0 ? data.aws_instances.controller_fleet_instances.private_ips : []

  asg_instance_ids = (local.controller_on_demand_count + local.controller_spot_count) > 0 ? data.aws_instances.controller_fleet_instances.ids : []
...
resource "null_resource" "bootstrap_kubernetes_control_plane" {
  count = (var.k8s_config.controller_on_demand_count + local.controller_spot_count) > 0 ? 1 : 0
...
locals {
  ...
  ssm_parameters_arns = [
    ...
    "arn:aws:ssm:${var.aws_config.environment}:${data.aws_caller_identity.current.account_id}:parameter/${param}"
  ]

  ssm_kms_arn = "arn:aws:kms:${var.aws_config.environment}:${data.aws_caller_identity.current.account_id}:alias/aws/ssm"
