...
output "vpc_endpoint_ids" {
  ...
  value = {
    ...
    ec2          = length(aws_vpc_endpoint.ec2) > 0 ? aws_vpc_endpoint.ec2[0].id : null
    ssm          = length(aws_vpc_endpoint.ssm) > 0 ? aws_vpc_endpoint.ssm[0].id : null
    ssm_messages = length(aws_vpc_endpoint.ssm_messages) > 0 ? aws_vpc_endpoint.ssm_messages[0].id : null
    ec2_messages = length(aws_vpc_endpoint.ec2_messages) > 0 ? aws_vpc_endpoint.ec2_messages[0].id : null
    logs         = length(aws_vpc_endpoint.logs) > 0 ? aws_vpc_endpoint.logs[0].id : null
    monitoring   = length(aws_vpc_endpoint.monitoring) > 0 ? aws_vpc_endpoint.monitoring[0].id : null
  }
}
