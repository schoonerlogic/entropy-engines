# modules/kubernetes/cpu-workers/variables.tf
# Variables for CPU worker module
...
#===============================================================================
# CPU Worker Instance Configuration
#===============================================================================

variable "cpu_instance_types" {
  description = "List of EC2 instance types for CPU workers"
  type        = list(string)
  default     = ["m6i.large", "m6i.xlarge", "m5.large", "m5.xlarge"]

  validation {
    condition     = length(var.cpu_instance_types) > 0
...
variable "cpu_on_demand_count" {
  ...
  validation {
    condition     = var.cpu_on_demand_count >= 0
...
variable "cpu_spot_count" {
  ...
  validation {
    condition     = var.cpu_spot_count >= 0
...
variable "instance_requirements" {
  description = "Instance requirements for CPU workers (used when use_instance_requirements is true)"
  type = object({
    vcpu_count = object({
      min = number
      max = number
...
variable "subnet_ids" {
  description = "List of subnet IDs where CPU workers will be launched"
  type        = list(string)
...
variable "security_group_ids" {
  description = "List of security group IDs for CPU workers"
  type        = list(string)
...
variable "ssh_key_name" {
  description = "Name of the AWS key pair for SSH access to CPU workers"
  type        = string
}

variable "worker_role_name" {
  description = "Name of the IAM role for CPU worker instances"
  type        = string
}
...
variable "min_size" {
  description = "Minimum size of the CPU worker Auto Scaling Group"
  type        = number
  default     = null
}

variable "max_size" {
  description = "Maximum size of the CPU worker Auto Scaling Group"
  type        = number
  default     = null
}
...
variable "additional_tags" {
  description = "Additional tags to apply to CPU worker resources"
  type        = map(string)
  default     = {}
}
...
variable "enable_cloudwatch_logs" {
  description = "Whether to enable CloudWatch logging for CPU workers"
  type        = bool
  default     = false
}

variable "enable_notifications" {
  description = "Whether to enable SNS notifications for CPU worker events"
  type        = bool
  default     = false
}
