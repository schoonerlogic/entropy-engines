...
module "controllers" {
  ...
  instance_types  = local.ctrl_config.instance_type
  ...
  pod_cidr_block             = local.crtl_config.pod_cidr_block
  service_cidr_block         = local.crtl_confg.service_cidr_block
  ...
  # IAM
  control_plane_role_name = local.crtl_config.control_plane_role_name
  ...
  # ASG configuration 
  health_check_grace_period = local.asg_config.health_check_grace_period
  min_healthy_percentage    = local.asg_config.min_healthy_percentage
  capacity_timeout          = local.asg_config.capacity_timeout
...
locals {
  gpu_config = {
    ...
    use_instance_requirements = var.gpu_config.use_instance_requirements
    instance_requirements = var.gpu_config.instance_requirements != null ? merge(
      var.gpu_config.instance_requirements,
      {
        # GPU-specific overrides
        instance_categories   = coalesce(var.gpu_config.instance_requirements.instance_categories, ["accelerated-computing"])
        ...
        accelerator_count = coalesce(
          var.gpu_config.instance_requirements.accelerator_count,
          { min = 1, max = 4 }
        )
        accelerator_manufacturers = (length(var.gpu_config.instance_requirements.accelerator_manufacturers) > 0 ?
        var.gpu_config.instance_requirements.accelerator_manufacturers : ["nvidia"])
        accelerator_types = (length(var.gpu_config.instance_requirements.accelerator_types) > 0 ?
        var.gpu_config.instance_requirements.accelerator_types : ["gpu"])

        # GPU workloads need high network performance
        network_bandwidth_gbps = coalesce(
          var.gpu_config.instance_requirements.network_bandwidth_gbps,
          { min = 10, max = null }
        )
      }
    ) : null
...
module "gpu_workers" {
  ...
  # Apply GPU-specific overrides or use base config
  health_check_grace_period = coalesce(
    local.gpu_config.gpu_asg_overrides.health_check_grace_period,
    local.gpu_config.asg_config.health_check_grace_period,
    600 # Longer for GPU drivers
  )
  min_healthy_percentage = coalesce(
    local.gpu_config.gpu_asg_overrides.min_healthy_percentage,
    local.gpu_config.asg_config.min_healthy_percentage,
    25 # Lower due to high cost
  )
  instance_warmup = coalesce(
    local.gpu_config.gpu_asg_overrides.instance_warmup,
    local.gpu_config.asg_config.instance_warmup,
    600 # GPU initialization time
  )

  # Storage configuration - use GPU overrides or base config
  block_device_mappings = (length(local.gpu_config.gpu_storage_overrides.block_device_mappings) > 0 ?
    local.gpu_config.gpu_storage_overrides.block_device_mappings :
  local.gpu_config.storage_config.block_device_mappings)

  # Worker role (can be different for GPU workers)
  worker_role_name = coalesce(
    local.iam_config.gpu_worker_role_name,
    local.iam_config.worker_role_name,
    "${local.cluster_name}-gpu-worker-role"
  )

  # GPU-specific settings
  gpu_type = local.gpu_config.gpu_type

  # Tags with GPU-specific additions
  additional_tags = merge(
    {
      WorkerType    = "gpu"
      Environment   = local.core_config.environment
      Project       = local.core_config.project
      CostTier      = "premium"
      HighCostAlert = "true"
      GPUType       = local.gpu_config.gpu_type
    },
    local.gpu_config.additional_tags
  )
}
