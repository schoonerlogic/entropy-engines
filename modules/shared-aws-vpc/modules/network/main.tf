...
resource "aws_security_group_rule" "cp_etcd_client" {
  security_group_id        = aws_security_group.k8s_control_plane.id
  type                     = "ingress"
  protocol                 = "tcp"
  from_port                = 2379
  ...
  description              = "etcd client communication between control planes"
}
...
resource "aws_security_group_rule" "cp_etcd_peer" {
  ...
  to_port                  = 2380
  source_security_group_id = aws_security_group.k8s_control_plane.id
  description              = "etcd peer communication between control planes"
}

# Worker Nodes Security Group
resource "aws_security_group" "k8s_nodes" {
  name        = local.nodes_sg_name
...
resource "aws_security_group_rule" "node_self_all" {
  ...
  protocol                 = "-1"
  source_security_group_id = aws_security_group.k8s_nodes.id
  description              = "Allow all traffic between nodes (CNI)"
}
...
resource "aws_security_group_rule" "bastion_ssh" {
  ...
  from_port         = 22
  to_port           = 22
  cidr_blocks       = length(local.effective_bastion_cidrs) > 0 ? local.effective_bastion_cidrs : (
    length(local.effective_ssh_cidrs) > 0 ? local.effective_ssh_cidrs : ["10.0.0.0/8"]
  )
